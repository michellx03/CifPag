...
public class AutorizadorInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object controller)
		...
		if (uri.endsWith("login") || uri.endsWith("Cifensa") || uri.contains("assets") || uri.contains("AcessoNegado")) {
			return true;
		}
		...
		if (request.getSession().getAttribute("usuarioLogado") != null) {
			...
			if (cam.usuarioLogadoTemAcesso(controllerNome, request) == false) {
				response.sendRedirect("/Cifensa/ControleAcesso/AcessoNegado");
				return false;
			}
			return true;
		}

		response.sendRedirect("/Cifensa");
		return false;
	}
}
